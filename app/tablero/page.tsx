...
export default function TableroPage() {
  ...
  const filteredTickets = useMemo(() => {
    if (!cliente) return tickets;
    return tickets.filter((t) => t.Cliente === cliente);
  }, [tickets, cliente]);
  ...
  const estadosList = useMemo(() => {
    const set = new Set(filteredTickets.map((t) => t.Estado).filter(Boolean));
    return Array.from(set).sort((a, b) => a.localeCompare(b));
  }, [filteredTickets]);
          ...
          <div className="overflow-x-auto pb-4">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className="text-xs font-medium text-zinc-700 dark:text-zinc-300">Cliente</span>
                <select
                  value={cliente}
                  onChange={(e) => setCliente(e.target.value)}
                  className="min-w-[180px] rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-3 py-1.5 text-sm text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
            ...
            {filteredTickets.length === 0 ? (
              <div className="rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700/80 bg-white/60 dark:bg-zinc-950/40 p-8 text-center text-sm text-zinc-500 dark:text-zinc-400">
                No hay tickets para el cliente seleccionado. Probá cambiar el filtro de cliente.
              </div>
            ) : (
